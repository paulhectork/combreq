"""
a small CLI to combine multiple python requirements files into a single one,
...
#TODO rework `rgx` for comma-separated version specs
...
def init_cli():
    parser = argparse.ArgumentParser('combine_requirements', description="a small CLI to combine multiple python requirements files into a single one, detecting dependendy conflicts along the way.")
...
def combine_requirements(
    ...
    """
    combine requirements file and detect version errors, if any
    ...
    ... ]
    >>> combine_requirements(reqs_list)
    ... # returns
    ... {
    ...     'wandb': [('', ''),
    ...     'pytorch': [('>=', 2.2), ('<=', 3.0)],
    ...
    :returns: combined requirements, as a dict
    """
...
if __name__ == "__main__":
    ...
    combined_reqs = combine_requirements(input_requirements)
    combined_reqs = to_string(combined_reqs)

    if output:
        write_file(output, combined_reqs)
    else:
        print(combined_reqs)
